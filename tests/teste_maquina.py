from projetin_xerox.programa.busca_elemento import click_sem_indice,escrever, retorna_text_indice_time,click_indice
...
def maquina_C400(driver):
    ...
    for vezes in range(0, 2):
        ...
        preto = retorna_text_indice_time(driver, locator=(By.CLASS_NAME, 'xux-labelableBox-content'), indice=5, time=10)
...
def abrir_teste():
    ...
    for c in lista:
        url = f'http://{c}'
        driver.get(url)
        toner.append(maquina_C400(driver))
    assert len(toner) == len(lista)
    print(toner)
